...
iris_data
iris_data.values
iris_data = iris_data.values #<FILL IN>
iris_data
...
euclidean_distance(unknown, test[0:4])
...
distances = [(euclidean_distance(unknown, flower[0:4]), flower[4]) for flower in iris_data] # <FILL IN>
...
nearest = [flower[1] for flower in distances[0:3]]
nearest
 # <FILL IN>
...
prediction = Counter(nearest).most_common(1)[0][0]
prediction    
'''
...
def predict(unknown, k = 3):
    ...
    # calculate distances
    distances = [(euclidean_distance(unknown, flower[0:4]), flower[4]) for flower in iris_data]
    
    # grab k nearest
    distances.sort()
    nearest = [flower[1] for flower in distances[0:k]]
    
    # count most common name
    prediction = Counter(nearest).most_common(1)[0][0]
